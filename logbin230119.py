...
def logbin(data, scale = 1., zeros = False,runs=1):
    """
    ...
    if scale > 1:
        ...
        y = np.zeros_like(x)
        yError = np.zeros_like(x)
        count = count.astype('float')
        for i in range(len(y)):
            y[i] = np.sum(count[binedges[i]:binedges[i+1]]/(binedges[i+1] - binedges[i]))
            yError[i] = ((np.sqrt(np.sum(count[binedges[i]:binedges[i+1]])))/(binedges[i+1] - binedges[i]))/np.sqrt(runs)            
